...
public:
  ...
  struct Entry
  {
    Entry(Value_t&& v, EventType e, bool start, int64_t ts, size_t parent) :
      d_value(std::move(v)), d_ts(ts), d_parent(parent), d_event(e), d_start(start)
    {
    }
    Entry(Value_t&& v, const std::string& custom, bool start, int64_t ts, size_t parent) :
      d_value(std::move(v)), d_custom(custom), d_ts(ts), d_parent(parent), d_event(CustomEvent), d_start(start)
    {
    }
    Value_t d_value;
    std::string d_custom;
  ...
  template <class E>
  size_t add(E e, Value_t&& v, bool start, int64_t stamp = 0)
  {
    assert(d_status != Invalid);
    ...
    stamp -= d_base;
    d_events.emplace_back(std::move(v), e, start, stamp, d_parent);
  ...
  size_t add(E e)
  {
    return add(e, Value_t(std::nullopt), true);
  }

  // We store uint32 in an int64_t
  template <class E>
  size_t add(E e, uint32_t v, bool start)
  {
    return add(e, static_cast<int64_t>(v), start);
  }
  // We store int32 in an int64_t
  template <class E>
  size_t add(E e, int32_t v, bool start)
  {
    return add(e, static_cast<int64_t>(v), start);
  }

  template <class E, class T>
  size_t add(E e, T v, bool start)
  {
    return add(e, Value_t(v), start);
  }
  ...
  public:
    ...
    void close(int64_t val)
    {
      ...
      d_eventTrace.add(d_event, val, false);
      d_closed = true;
    }
