...
pub fn serveweb(addresses: &Vec<String>, urls: &[String], password_ch: cxx::UniquePtr<rustweb::CredentialsHolder>, api_ch: cxx::UniquePtr<rustweb::CredentialsHolder>, acl: cxx::UniquePtr<rustweb::NetmaskGroup>) -> Result<(), std::io::Error> {
    ...
    let ctx = Arc::new(Context {
        ...
        api_ch,
        acl,
        counter: Mutex::new(0), // more for educationl purposes
    });

    // We use a single thread to handle all the requests, luckit the runtime abstracts from this
...
// impl below needed because the classes are used in the Context, which gets passes around.
