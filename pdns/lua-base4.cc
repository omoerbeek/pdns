...
void BaseLua4::loadFile(const std::string& fname, bool doPostLoad)
{
  ...
  if (!ifs) {
    ...
    SLOG(g_log << Logger::Error << "Unable to read configuration file from '" << fname << "': " << msg << endl,
         g_slog->withName("lua")->error(Logr::Error, ret, "Unable to read configuration file", "filename", Logging::Loggable(fname))); 
...
void BaseLua4::includePath(const std::string& directory) {
    ...
    if (boost::ends_with(name, suffix)) {
      ...
      if (stat(fullName.c_str(), &statInfo) != 0 || !S_ISREG(statInfo.st_mode)) {
        ...
        SLOG(g_log << Logger::Error << msg << std::endl,
             g_slog->withName("lua")->info(Logr::Error, "File is not a regular file", "filename", Logging::Loggable(fullName)));
  ...
  if (directoryError) {
    ...
    SLOG(g_log << Logger::Error << msg << std::endl,
         g_slog->withName("lua")->error(Logr::Error, err, "Directory is not accessible", "directory", Logging::Loggable(directory)));
    throw PDNSException(std::move(msg));
  }
